...
#1.加载数据集
(x,y) , (x_test,y_test) = datasets.mnist.load_data(path = r'./mnist.npz')
...
for i in range(0,num_iretation):
    ...
    for (x_test_minibatch,y_test_minibatch) in test_db:
        x_test_minibatch = tf.reshape(x_test_minibatch,[-1,28*28])
        #前向计算
        z1 = x_test_minibatch @ W1 +b1
            #激活函数 
        a1 = tf.nn.relu(z1)
        z2 = a1 @ W2 + b2
        a2 = tf.nn.relu(z2)
        ...
        pred = tf.argmax(a3,axis = -1)  #得到概率最大值的索引 所以这里得到的就是正确答案
        pred = tf.cast(pred,dtype = tf.int32)
        print(pred.shape)
        print(y_test_minibatch.shape)
        correct = tf.cast(tf.equal(pred,y_test_minibatch),dtype = tf.int32)
        
        
        correct = tf.reduce_sum(correct)
        
        total_correct += int(correct)
        total_num += x_test_minibatch.shape[0]  
    
    accuracy.append(total_correct/total_num)
    print("The " + str(i) +" epoch: Accuracy is {} ".format(float(accuracy[len(accuracy)-1])))
