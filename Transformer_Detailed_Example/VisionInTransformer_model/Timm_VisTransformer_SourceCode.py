...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0.):
        super().__init__()
        assert dim % num_heads == 0, 'dim should be divisible by num_heads'  # emb_dim的维度必须要能被头数量整除
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.cls_token = nn.Parameter(torch.zeros(1, 1, embed_dim)) if class_token else None  # CLS 表头初始化
        embed_len = num_patches if no_embed_class else num_patches + self.num_prefix_tokens
        self.pos_embed = nn.Parameter(torch.randn(1, embed_len, embed_dim) * .02)  # 位置编码位置初始化  注意这里都是 nn.Parameter之后直接送入模型，不含有任何人工固定过程
        #   一个模型只会定义一个 pos_embed 和 CLS_TOKEN  因为只有初始含有这些数字  我们看一下  cls_token 和  pos_emb 每次的结果
        if (DEBUG_MODE):
            print("cls_token的初始化内容位{}".format( self.cls_token[0,0,0:24]))
            print("pos_embed的初始化内容为{}".format( self.pos_embed[0,0:3,0:15]))
    ...
    def _pos_embed(self, x):
        if self.no_embed_class:
            ...
            # 如果存在 CLS_TOKEN 就将 已经经过初始化的 cls_token 直接cat  然后和 pos_embed 相加
            if self.cls_token is not None:
                x = torch.cat((self.cls_token.expand(x.shape[0], -1, -1), x), dim=1)
            x = x + self.pos_embed
    ...
    def forward_features(self, x):
        if (DEBUG_MODE):
            print("图片输入模型的形状{}".format(x.size()))
        x = self.patch_embed(x)
        if (DEBUG_MODE):
            print("图片经过patch_embed的形状{}".format(x.size()))
        x = self._pos_embed(x)
        if (DEBUG_MODE):
            print("位置编码之后的形状{}".format(x.size()))
        x = self.norm_pre(x)
    ...
    def forward_head(self, x, pre_logits: bool = False):
        if self.global_pool:
            x = x[:, self.num_prefix_tokens:].mean(dim=1) if self.global_pool == 'avg' else x[:, 0] # 这里的代码可以很明显的看出 到底是使用 cls 还是 直接对剩下的做Mean
        x = self.fc_norm(x)
...
@register_model
def vit_base_patch32_224_Debug(pretrained=False,DEBUG_MODE = False, **kwargs):
    """ ViT-Base (ViT-B/32) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-1k weights fine-tuned from in21k, source https://github.com/google-research/vision_transformer.
    """
    model_kwargs = dict(patch_size=32, embed_dim=768, depth=12, num_heads=12, **kwargs)
    if (DEBUG_MODE):
        print("*"*10, "传入模型的定义参数", "*" * 10)
        print(model_kwargs)
    model = _create_vision_transformer('vit_base_patch32_224', pretrained=pretrained, **model_kwargs)
    return model
