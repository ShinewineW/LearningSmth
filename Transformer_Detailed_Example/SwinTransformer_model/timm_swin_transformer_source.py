...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # partition windows
        if (DEBUG_MODE):
            print("张量在 通过窗口划分之前的形状{}".format(shifted_x.size()))
        x_windows = window_partition(shifted_x, self.window_size)  # num_win*B, window_size, window_size, C
        if (DEBUG_MODE):
            print("张量在 通过窗口划分之后的形状{}".format(x_windows.size()))
...
class SwinTransformer(nn.Module):
    ...
    def forward_features(self, x):
        ...
        if(DEBUG_MODE):
            print("初始张量经过 patch_embed 后形状变为{}".format(x.size()))
        if self.absolute_pos_embed is not None:
...
def swin_base_patch4_window7_224(pretrained=False, **kwargs):
    ...
    if (DEBUG_MODE):
        print("输入默认参数为：{}".format(model_kwargs))
